...
class StackedBarGrapher:
    ...
    def _normalise_list(self, lst, norm_min=None, norm_max=None):    
        if norm_max == None:
            norm_max = np.max(lst)    
        if norm_min == None:
            norm_min = np.min(lst)    
    ...
    def autolabel(self, rects, ax, max_dsize, arr_sums,f):        
        ...
        if arr_sums == None:
            ...
            for rect in rects:
                ...
                #norm_height = ((height-min_v)/(max_v-min_v))
                norm_height = (height/max_dsize)*f
                
                if norm_height > 0.5:
                    rec_y = rect.get_y()
                    ax.text(rect.get_x() + rect.get_width()/2., (height)/2.0+(rec_y),
                            "%.1f%%" % norm_height,
                            ha='center', va='bottom', fontsize=11)
        else:
            #print "here"
            # attach some text labels
            for rect, mxs in zip(rects, arr_sums):
                #print "here -"
                height = rect.get_height()
                #norm_height = ((height-min_v)/(max_v-min_v))
                norm_height = (height/1.0)*f
                #norm_height = (height/mxs)*f
                #print height, mxs, f 
                ...
                if norm_height > 0.0:
                    rec_y = rect.get_y()
                    ax.text(rect.get_x() + rect.get_width()/2., (height)/2.5+(rec_y),
                            "%.1f%%" % norm_height,
                            ha='center', va='bottom', fontsize=11)
    ...
    def stackedBarPlot(self,
        ...
        if arr_sums == None:
            print "sum_data : ", np.sum(data, axis=1)        
            max_data_size = np.sum(data, axis=1)[0]
        else:
            max_data_size = 0.0
...
# plot
        ...
        self.autolabel(rects, ax, max_data_size, arr_sums,f)
        ...
        if yTicks is not "none":
            ax.tick_params(axis='y', which='both', labelsize=11, direction="out")
            ax.yaxis.tick_left()
        ...
        if xLabels is not None:
            ax.tick_params(axis='x', which='both', labelsize=11, direction="out")
            ax.xaxis.tick_bottom()
            #plt.xticks(x, xLabels, rotation='vertical')
            ax.set_xticks(x)
            #ax.set_xticklabels(xLabels, rotation=15, fontsize=11)
